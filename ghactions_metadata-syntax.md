...
可创建操作来执行存储库中的任务。 如果要执行自定义操作，则需要使用 YAML 语法的元数据文件。

<!-- TRANSLATION_FALLBACK prop=markdown type=ParseError line=753 col=1 msg="tag {% ifversion fpt or ghec %} not closed" -->
...
Workflow files that use this action can use the `with` keyword to set an input value for `octocat-eye-color`. For more information about the `with` syntax, see [Workflow syntax for GitHub Actions](/en/actions/using-workflows/workflow-syntax-for-github-actions#jobsjob_idstepswith).
...
If the action is written using a [composite](/en/actions/creating-actions/creating-a-composite-action), then it will not automatically get `INPUT_<VARIABLE_NAME>`. With composite actions you can use `inputs` [Contexts reference](/en/actions/learn-github-actions/contexts) to access action inputs.

To access the environment variable in a Docker container action, you must pass the input using the `args` keyword in the action metadata file. For more information about the action metadata file for Docker container actions, see [Creating a Docker container action](/en/actions/creating-actions/creating-a-docker-container-action#creating-an-action-metadata-file).
...
If you don't declare an output in your action metadata file, you can still set outputs and use them in a workflow. For more information on setting outputs in an action, see [Workflow commands for GitHub Actions](/en/actions/using-workflows/workflow-commands-for-github-actions#setting-an-output-parameter).
...
For more information on how to use context syntax, see [Contexts reference](/en/actions/learn-github-actions/contexts).
...
For more information, see [Contexts reference](/en/actions/learn-github-actions/contexts#github-context).

#### `runs.steps[*].shell`

**Optional** The shell where you want to run the command. You can use any of the shells listed in [Workflow syntax for GitHub Actions](/en/actions/using-workflows/workflow-syntax-for-github-actions#jobsjob_idstepsshell). Required if `run` is set.
...
The `my backup step` only runs when the previous step of a composite action fails. For more information, see [Evaluate expressions in workflows and actions](/en/actions/learn-github-actions/expressions#status-check-functions).
...
**Optional** A unique identifier for the step. You can use the `id` to reference the step in contexts. For more information, see [Contexts reference](/en/actions/learn-github-actions/contexts).
...
Some actions require inputs that you must set using the [`with`](/en/actions/using-workflows/workflow-syntax-for-github-actions#jobsjob_idstepswith) keyword. Review the action's README file to determine the inputs required.
...
For more information about how the `entrypoint` executes, see [Dockerfile support for GitHub Actions](/en/actions/creating-actions/dockerfile-support-for-github-actions#entrypoint).
...
For more information about using the `CMD` instruction with GitHub Actions, see [Dockerfile support for GitHub Actions](/en/actions/creating-actions/dockerfile-support-for-github-actions#cmd).
